...
A 3D CSS renderer draws iFrames in 3D space. Over that, a WebGL renderer displays the 3D level, making sure to avoid rendering over the iFrames (done using occlusionObjects array in engine.js). Raycasts are performed every frame to check if the mouse is over a panel, in which case the WebGL layer's pointer events are disabled to allow for interacting with the iFrame. When an iFrame is partially obstructed, the html within it should bubble any mousemove events to the main window to update the raycast position (see guestbook.html for an implementation).
...
level.js defines the 3D scene, including the iFrames. In order for the player to be able to collide with any geometry, that mesh has to be pushed to the collisionObjects list. To save on resources, invisible simplified geometry should be pused instead. In the case of panels (iFrames) their first child object, being the occlusion mesh, should be added to the list.